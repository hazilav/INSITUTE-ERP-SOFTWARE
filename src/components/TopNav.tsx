...
export default function TopNav({ user, institute, onOpenSidebar }: TopNavProps) {
  ...
  return (
    <>
      <header className="h-16 bg-white border-b border-slate-200/80 sticky top-0 z-30 px-2.5 sm:px-4 lg:px-6 flex items-center justify-between shadow-xs">
        {/* Left section */}
        <div className="flex items-center gap-1.5 sm:gap-3">
          <button
            onClick={onOpenSidebar}
            className="p-2 text-slate-500 hover:text-slate-700 hover:bg-slate-100 rounded-xl lg:hidden transition-colors min-w-[40px] min-h-[40px] flex items-center justify-center"
        ...
        <div
          ...
          className="flex items-center justify-between cursor-pointer max-w-md flex-1 min-w-0 mx-1.5 sm:mx-4 px-2 sm:px-3.5 py-1.5 bg-slate-100/80 hover:bg-slate-100 border border-slate-200/80 rounded-xl text-xs text-slate-500 transition-all shadow-xs min-h-[38px]"
        >
          <div className="flex items-center gap-1.5 sm:gap-2 min-w-0 truncate">
            <Search className="w-4 h-4 text-brand-600 shrink-0" />
            <span className="font-medium text-slate-600 truncate text-[11px] sm:text-xs">Search...</span>
          </div>
